...
public:
    ...
    private:
        int mMaxCycleLength = std::numeric_limits<int>::max();
    };
...
private:
    ...
    void _process(const size_t ij, const size_t ik, const size_t kj, const int halfLength);
...
vspc::SSSR::Path convertGraphToPath(vspc::UndirectedGraph graph)
{
    ...
    while (graph.prune().numNodes() != 0) {
        ...
        if (conn.empty()) {
            for (const auto& uGraphElem : graph) {
                const NodeType& n = uGraphElem.first;
                const std::set<NodeType>& c = uGraphElem.second;
...
SSSR::_initializePID()
{
    ...
    int halfLength = mSettings.getMaxCycleLength() / 2;
    if (mSettings.getMaxCycleLength() % 2 == 1) {
        ++halfLength;
    }
...
SSSR::_convertNodeIndices()
{
    ...
    for (size_t i = 0, n = mSSSR.size(); i < n; ++i) {
        for (const auto& uGraphElem : mSSSR[i]) {
            const NodeType& node = uGraphElem.first;
            const std::set<NodeType>& conn = uGraphElem.second;
...
void
SSSR::_process(const size_t ij, const size_t ik, const size_t kj, const int halfLength)
{
